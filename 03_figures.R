...
fit <- readRDS("output/fitted_model_LAG7_RE.rds")
end_day <- "2020-08-06"
...
# Random effects --------------------------------------------------------------- 
# (exponentiated)
# multiplicative effect on \lambda_i

if(!is.null(ranef(fit))) {
  reff_mean <- exp(ranef(fit))
  reff_sd <- ranef(fit, se = T)[, 2]
  
  
  africa$reff_mean_AR <- as.numeric(reff_mean[grep("ar.ri", names(reff_mean))])
  africa$reff_sd_AR <- as.numeric(reff_sd[grep("ar.ri", names(reff_sd))])
  
  africa_full <- africa_full %>% 
    full_join(st_drop_geometry(africa[c("name", "reff_mean_AR", "reff_sd_AR")]))
  
  if(length(grep("ne.ri", names(reff_mean))) > 0) {
    africa$reff_mean_NE <- as.numeric(reff_mean[grep("ne.ri", names(reff_mean))])
    africa$reff_sd_NE <- as.numeric(reff_sd[grep("ne.ri", names(reff_sd))])
    
    africa_full <- africa_full %>% 
      full_join(st_drop_geometry(africa[c("name", "reff_mean_NE", "reff_sd_NE")]))
  }
  
  
  if(length(grep("ar.ri", names(reff_mean))) > 0) {
    ...
    reff_sd_AR <- map(x = "reff_sd_AR", shape = africa_full, palette = "Greens", 
                            n = 7, digits = 2, legend_title = "Standard\nErrors", 
                            panel_title = "Random Effects - AR")
    
    
    reff_map <- tmap_arrange(reff_mean_AR, reff_sd_AR, ncol = 2)
    tmap_save(reff_map, "figs/reff_AR_LAG.png",
              width = 7 * 2, height = 8)
  }
  ...
  if(length(grep("ne.ri", names(reff_mean))) > 0) {
    ...
    reff_sd_NE <- map(x = "reff_sd_NE", shape = africa_full, palette = "Greens", 
                            n = 7, digits = 2, legend_title = "Standard\nErrors", 
                            panel_title = "Random Effects - NE")
    
    reff_map <- tmap_arrange(reff_mean_NE, reff_sd_NE, ncol = 2)
    tmap_save(reff_map, "figs/reff_NE_LAG.png",
              width = 7 * 2, height = 8)
    
    
    reff_map <- tmap_arrange(reff_mean_AR, reff_mean_NE,
                             ncol = 2)
    tmap_save(reff_map, "figs/paper/reff_map.pdf",
              width = 7 * 2, height = 8)
  }
}
...
ggsave("figs/paper/contribution_time.pdf", width = 6, height = 8, dpi = 320)
...
wb_map <- tmap_arrange(within, between, ncol = 2)
tmap_save(wb_map, "figs/wb_map_LAG.png", 
          width = 7 * 2, height = 8)

ew_map <- tmap_arrange(endemic, within, ncol = 2)
tmap_save(wb_map, "figs/ew_map_LAG.png", 
          width = 7 * 2, height = 8)

wbe_map <- tmap_arrange(endemic, within, between, ncol = 3)
tmap_save(wbe_map, "figs/wbe_map_LAG.png",
          width = 7 * 3, height = 8)

tmap_save(within, "figs/paper/contribution_within_space.pdf",
          width = 7, height = 8)
...
# ggplot(quants_insample, aes(x = time)) +
#   geom_ribbon(aes(ymin = low, ymax = up), fill = "dodgerblue3", alpha = .5) +
#   geom_line(aes(y = observed), linetype = 2) +  
#   geom_line(aes(y = median), col = "dodgerblue3", size = 1) +
#   geom_point(aes(y = observed), shape = 21) + 
#   facet_wrap(~ LTLA, scales = "free_y") +
#   labs(x = "", y = "Reported cases") +
#   scale_x_date(date_breaks = "1 week", date_labels = "%b %d")
# 
# 
# ggsave("figs/predictions_ltlas.png", width = 8, height = 6, dpi = 320)
...
cols_to_replace <-c("low95", "low50", "mean", "median", "up50", "up95")
sims[sims$time == pred_start, cols_to_replace] <- 
  quants_insample[quants_insample$time == pred_start, cols_to_replace]
...
ggplot(all_preds, aes(x = time)) +
  ...
  geom_point(aes(y = observed, shape = type), size = 2.5) + 
  facet_wrap(~ cname, scales = "free_y") +
  labs(x = "", y = "Reported Cases") +
...
ggsave("figs/paper/predictions.pdf", width = 20, height = 10, dpi = 320)
...
ggsave("figs/paper/contribution_time_ALL.pdf", 
       width = 6 * 5, height = 14, dpi = 320)



# Predictions all Countries ----------------------------------------------------
pred <- oneStepAhead_hhh4lag(result = fit, tp = c(start_day, end_day) - 1, 
                             type = "final")
...
ggsave("figs/paper/predictions_ALL.pdf", 
